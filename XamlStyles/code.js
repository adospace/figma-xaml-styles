        ...
        if (msg.type === 'generate-styles') {
            ...
            figma.getLocalPaintStyles()
                ...
                paintStyle.paints.forEach(paint => {
                    if (paint.type === "SOLID") {
                        data.colors.push(generateSolidBrush((paintStyle.paints.length > 1) ? `${paintStyle.name}/${camelize(paint.blendMode)}` : paintStyle.name, paint));
                    }
                    else if (paint.type === 'GRADIENT_LINEAR') {
                        data.colors.push(generateGradientBrush((paintStyle.paints.length > 1) ? `${paintStyle.name}/${camelize(paint.blendMode)}` : paintStyle.name, paint));
                    }
                });
            });
    ...
    figma.ui.onmessage = msg => {
        if (msg.type === 'generate-styles') {
            ...
            figma.getLocalPaintStyles()
                ...
                paintStyle.paints.forEach(paint => {
                    if (paint.type === "SOLID") {
                        data.colors.push(generateSolidBrush(paintStyle.name, paint));
                    }
                    else if (paint.type === 'GRADIENT_LINEAR') {
                        data.colors.push(generateGradientBrush(paintStyle.name, paint));
                    }
                });
            });
            //console.log("sending message to ui:");
...
;
function generateSolidBrush(name, solidPaint) {
...
function generateGradientBrush(name, gradientPaint) {
    name = name.replace(/[&\/\\#,+()$~%.'":;*?<>{}\s]/g, "");
